...
geohash-kit includes comprehensive benchmarks for all major functions, co-located with source modules (`*.bench.ts`
files). Benchmarks measure real-world performance across varying input sizes and parameters to identify hotspots and
track regressions.
...
All benchmarks run on a modern development machine. Mobile and older devices will be slower (
see [Performance on Different Devices](#performance-on-different-devices)).
...
- **`polygonToGeohashes`** — operator defines service area on a map. One-time operation, not performance-critical. Even
  100ms is acceptable.
...
Means: ~1,000 polygon coverages per second. Average time per operation is 0.97ms. 75% of operations complete in <1.0ms,
99% in <1.4ms. The measurement uncertainty is ±1%.
...
These benchmarks run on **Apple Silicon 2024** (high-end development machine). Real-world performance varies
significantly:
...
Benchmarks take ~60 seconds to run and are resource-intensive. Include in CI/CD only if you need performance tracking
across commits.
